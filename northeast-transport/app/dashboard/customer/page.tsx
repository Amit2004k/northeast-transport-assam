...
import {
  ...
  Truck, Phone, Star, MapPin, ChevronRight,
...
const MOCK_BOOKINGS = [
  {
    ...
    drop_address: 'Jorhat City Center',
    distance_km: 312,
    estimated_price: 5766,
    vehicle_type: 'tata_yodha',
    status: 'completed',
    requested_at: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(),
    vehicles: { display_name: 'Tata Yodha', registration_number: 'AS01AB1234' },
    driver: { full_name: 'Bikash Sharma', phone: '+91 98765 43210' },
    rating: 5,
  },
  {
    id: 'BK-002',
    ...
    vehicles: { display_name: 'Bolero Pickup', registration_number: 'AS01CD5678' },
    driver: { full_name: 'Ranjit Das', phone: '+91 87654 32109' },
    rating: null,
  },
  {
    id: 'BK-003',
    pickup_address: 'Ganeshguri, Guwahati',
    drop_address: 'Beltola Market',
    distance_km: 5.4,
    estimated_price: 181,
      ...
      <div className="bg-dark-800 border-b border-white/5 px-4 pt-12 pb-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-dark-700 rounded-xl p-3 text-center">
            ...
            <div className="text-white/40 text-xs">Trips</div>
          </div>
          <div className="bg-dark-700 rounded-xl p-3 text-center">
      ...
      {activeBookings.length > 0 && activeBookings[0].status === 'in_progress' && (
        <div className="mx-4 mt-4 bg-gradient-to-r from-brand-500/20 to-brand-700/10 border border-brand-500/30 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-brand-400 font-medium text-sm">Booking in progress</span>
          </div>
          <div className="space-y-1.5 mb-3">
            <div className="flex items-center gap-2 text-sm">
              <MapPin size={14} className="text-brand-400" />
              <span className="text-white/70 truncate">{activeBookings[0].pickup_address}</span>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <div className="w-3.5 h-3.5 rounded-full border-2 border-emerald-400" />
              <span className="text-white/70 truncate">{activeBookings[0].drop_address}</span>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 bg-dark-700 rounded-full flex items-center justify-center text-sm font-bold">
                  {activeBookings[0].driver.full_name[0]}
                </div>
                <span className="text-sm">{activeBookings[0].driver.full_name}</span>
              </div>
              <a
                href={`tel:${activeBookings[0].driver.phone}`}
                className="flex items-center gap-1.5 bg-emerald-500/20 text-emerald-400 px-3 py-1.5 rounded-full text-xs border border-emerald-500/20"
              >
                <Phone size={12} />
                Call Driver
              </a>
        ...
        <div className="flex gap-1 bg-dark-800 rounded-xl p-1">
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
              activeTab === 'active' ? 'bg-dark-600 text-white' : 'text-white/40'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
              activeTab === 'history' ? 'bg-dark-600 text-white' : 'text-white/40'
            }`}
          >
      ...
      <div className="px-4 mt-4 space-y-3">
        ...
        {activeTab === 'active' && activeBookings.length === 0 && (
          <div className="text-center py-12">
            ...
            <Link href="/booking" className="btn-primary inline-flex mt-4 text-sm">
              Book a Vehicle
            </Link>
          </div>
        )}
...
function BookingCard({ booking }: { booking: any }) {
  const vehicleEmojis: Record<string, string> = {
    tata_yodha: '🚛', bolero_pickup: '🚚', mini_truck: '🚛',
    mahindra_pickup: '🚐', tata_ace: '🚐',
  }
  ...
  return (
    <div className="card">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-xl">{vehicleEmojis[booking.vehicle_type] || '🚚'}</span>
          <div>
            <p className="font-medium text-sm">{booking.vehicles?.display_name || booking.vehicle_type}</p>
            <p className="text-white/30 text-xs">{new Date(booking.requested_at).toLocaleDateString('en-IN')}</p>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between border-t border-white/5 pt-2">
        <div className="flex items-center gap-3">
          <span className="text-xs text-white/40">{booking.distance_km} km</span>
          <span className="font-semibold text-brand-400 text-sm">₹{booking.final_price || booking.estimated_price}</span>
        </div>
        {booking.driver && (
          <div className="flex items-center gap-1 text-xs text-white/50">
            <span>{booking.driver.full_name}</span>
            {booking.vehicles?.registration_number && (
              <span className="text-white/30">• {booking.vehicles.registration_number}</span>
            )}
          </div>
        )}
      </div>
      ...
      {booking.rating && (
        <div className="flex items-center gap-1 mt-2">
          {Array(booking.rating).fill(0).map((_: any, i: number) => (
