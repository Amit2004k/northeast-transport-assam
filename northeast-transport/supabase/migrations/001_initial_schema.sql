...
-- Documents policies
CREATE POLICY "Drivers see own documents" ON public.documents
  FOR SELECT USING (auth.uid() = driver_id);

CREATE POLICY "Drivers upload own documents" ON public.documents
  FOR INSERT WITH CHECK (auth.uid() = driver_id);

-- Driver earnings policies
CREATE POLICY "Drivers see own earnings" ON public.driver_earnings
  FOR SELECT USING (auth.uid() = driver_id);
...
-- =====================
-- SEED DATA (Demo vehicles in Guwahati)
-- =====================
-- Note: Seed data requires actual user IDs. Run seed script separately.
