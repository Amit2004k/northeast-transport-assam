...
import {
  ArrowLeft, MapPin, Navigation, Truck, Package,
  ChevronRight, Check, Loader2, Clock, IndianRupee
} from 'lucide-react'
...
export default function BookingPage() {
  ...
  const allLocations = [...GUWAHATI_LOCATIONS, ...ASSAM_CITIES.map(c => ({
    name: c.name, lat: c.lat, lng: c.lng
  }))]

  const filteredPickup = allLocations.filter(l =>
    l.name.toLowerCase().includes(pickupSearch.toLowerCase())
  ).slice(0, 6)

  const filteredDrop = allLocations.filter(l =>
    l.name.toLowerCase().includes(dropSearch.toLowerCase())
  ).slice(0, 6)
  ...
  const distance = pickup.lat && drop.lat
    ? calculateDistance(pickup.lat, pickup.lng, drop.lat, drop.lng)
    : 0

  const priceEstimate = selectedVehicle && distance > 0
    ? calculatePrice(distance, selectedVehicle)
    : null
  ...
  return (
    <main className="min-h-screen bg-dark-900 flex flex-col max-w-lg mx-auto">
      ...
      <div className="sticky top-0 z-10 bg-dark-900/90 backdrop-blur-xl border-b border-white/5 px-4 py-4 flex items-center gap-3">
        {step !== 'success' ? (
          <button
            onClick={() => step === 'location' ? router.back() : setStep(step === 'vehicle' ? 'location' : 'vehicle')}
        ...
        <div>
          ...
          {step !== 'success' && (
            <p className="text-white/40 text-xs">
              Step {step === 'location' ? 1 : step === 'vehicle' ? 2 : 3} of 3
            </p>
          )}
        </div>
      </div>
      ...
      {step !== 'success' && (
        <div className="h-1 bg-dark-700">
          <div
            ...
            style={{ width: step === 'location' ? '33%' : step === 'vehicle' ? '66%' : '100%' }}
          />
        </div>
      )}
      ...
      <div className="flex-1 px-4 py-6 overflow-y-auto">
        ...
        {step === 'location' && (
          <div className="space-y-4 animate-slide-up">
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={e => {
                        ...
                        if (e.target.value === '') setPickup({ address: '', lat: 26.1445, lng: 91.7362 })
                      }}
                      onFocus={() => setShowPickupSuggestions(true)}
                    />
                    {showPickupSuggestions && pickupSearch && filteredPickup.length > 0 && (
                      <div className="absolute top-full left-0 right-0 mt-1 bg-dark-700 border border-white/10 rounded-xl overflow-hidden z-20 shadow-xl">
                        {filteredPickup.map(loc => (
                          <button
                            key={loc.name}
                            className="w-full text-left px-4 py-3 text-sm hover:bg-dark-600 flex items-center gap-2 transition-colors"
                            onClick={() => {
                              setPickup({ address: loc.name, lat: loc.lat, lng: loc.lng })
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={e => {
                        ...
                        if (e.target.value === '') setDrop({ address: '', lat: 0, lng: 0 })
                      }}
                      onFocus={() => setShowDropSuggestions(true)}
                    />
                    {showDropSuggestions && dropSearch && filteredDrop.length > 0 && (
                      <div className="absolute top-full left-0 right-0 mt-1 bg-dark-700 border border-white/10 rounded-xl overflow-hidden z-20 shadow-xl">
                        {filteredDrop.map(loc => (
                          <button
                            ...
                            className="w-full text-left px-4 py-3 text-sm hover:bg-dark-600 flex items-center gap-2 transition-colors"
                            onClick={() => {
                              setDrop({ address: loc.name, lat: loc.lat, lng: loc.lng })
            ...
            {distance > 0 && (
              <div className="card flex items-center justify-between animate-fade-in">
                <div className="flex items-center gap-2">
                  <Clock size={16} className="text-brand-400" />
                  <span className="text-sm text-white/60">Estimated distance</span>
                </div>
                <span className="font-semibold text-brand-400">{distance.toFixed(1)} km</span>
              </div>
            )}
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {GUWAHATI_LOCATIONS.slice(0, 5).map(loc => (
                  <button
                    key={loc.name}
                    onClick={() => {
                      if (!pickup.address) {
                    ...
                    className="px-3 py-1.5 bg-dark-700 border border-white/5 rounded-full text-xs text-white/60 hover:border-brand-500/30 hover:text-white transition-all"
                  >
                    {loc.name}
                  </button>
                ))}
        ...
        {step === 'vehicle' && (
          <div className="space-y-3 animate-slide-up">
            <p className="text-white/40 text-sm">
              {pickup.address} → {drop.address} • {distance.toFixed(1)} km
            </p>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-white/40 text-xs">{config.capacityKg} kg capacity</div>
                        <div className="text-white/30 text-xs mt-0.5">{config.description}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className={`font-display font-bold text-lg ${isSelected ? 'text-brand-400' : 'text-white'}`}>
                        ₹{price.totalFare}
                      </div>
                      <div className="text-white/40 text-xs">₹{config.pricePerKm}/km</div>
                      {isSelected && (
                        <div className="mt-1 w-5 h-5 bg-brand-500 rounded-full flex items-center justify-center ml-auto">
            ...
            {selectedVehicle && (
              <div className="card animate-slide-up space-y-3">
                ...
                <input
                  type="text"
                  placeholder="What are you transporting? e.g., furniture, goods..."
                  className="input-field text-sm"
                  value={goodsDesc}
                ...
                <input
                  type="number"
                  placeholder="Weight in kg (approx)"
                  className="input-field text-sm"
                  value={weightKg}
        ...
        {step === 'confirm' && selectedVehicle && priceEstimate && (
          <div className="space-y-4 animate-slide-up">
            <div className="card">
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 bg-brand-500/20 rounded-full flex items-center justify-center shrink-0 mt-0.5">
                    <MapPin size={14} className="text-brand-400" />
                  </div>
                  <div>
                    <div className="text-xs text-white/40">Pickup</div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-xs text-white/40">Drop</div>
                    <div className="text-sm font-medium">{drop.address}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-dark-700 rounded-xl flex items-center justify-center text-2xl">
                  {VEHICLE_CONFIGS[selectedVehicle].emoji}
                </div>
                <div>
                  <div className="font-semibold">{VEHICLE_CONFIGS[selectedVehicle].displayName}</div>
                  <div className="text-white/40 text-xs">{VEHICLE_CONFIGS[selectedVehicle].capacityKg} kg capacity</div>
                </div>
              </div>
            </div>
            ...
            {goodsDesc && (
              <div className="card">
                <h3 className="font-semibold text-sm text-white/60 mb-2">Goods</h3>
                <p className="text-sm">{goodsDesc}</p>
                {weightKg && <p className="text-white/40 text-xs mt-1">{weightKg} kg</p>}
              </div>
            )}
            ...
            <div className="card">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-white/60">Distance ({priceEstimate.distanceKm} km)</span>
                  <span>₹{priceEstimate.distanceFare}</span>
                </div>
                <div className="h-px bg-white/10" />
                <div className="flex justify-between font-bold">
                  <span>Total Estimate</span>
                  <span className="text-brand-400 text-lg">₹{priceEstimate.totalFare}</span>
                </div>
              </div>
        ...
        {step === 'success' && (
          <div className="text-center space-y-6 pt-8 animate-slide-up">
            ...
            <div>
              ...
              <p className="text-white/50 text-sm">Finding the nearest available driver...</p>
            </div>

            {bookingId && (
              <div className="card">
                <p className="text-white/40 text-xs mb-1">Booking ID</p>
                <p className="font-mono font-bold text-brand-400">{bookingId}</p>
              </div>
            )}
            ...
            <div className="card text-left space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">Driver will be assigned shortly</span>
              </div>
              <div className="flex items-center gap-2 opacity-50">
                <div className="w-2 h-2 bg-white/20 rounded-full" />
                <span className="text-sm">Driver heading to pickup</span>
              </div>
              <div className="flex items-center gap-2 opacity-30">
                <div className="w-2 h-2 bg-white/20 rounded-full" />
                <span className="text-sm">Goods in transit</span>
              </div>
              <div className="flex items-center gap-2 opacity-20">
                <div className="w-2 h-2 bg-white/20 rounded-full" />
                <span className="text-sm">Delivery complete</span>
              </div>
            </div>

            <div className="flex flex-col gap-3">
              <Link href="/dashboard/customer" className="btn-primary text-center">
                Track Booking
              </Link>
              <button
                onClick={() => {
                  setStep('location')
                  setPickup({ address: '', lat: 26.1445, lng: 91.7362 })
