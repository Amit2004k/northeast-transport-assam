...
export async function POST(request: NextRequest) {
  ...
  if (!Object.keys(VEHICLE_CONFIGS).includes(vehicleType)) {
  ...
  return NextResponse.json({
    data: {
      ...estimate,
      vehicleType,
      vehicleName: VEHICLE_CONFIGS[vehicleType as keyof typeof VEHICLE_CONFIGS].displayName,
    }
  })
}
...
export async function GET(request: NextRequest) {
  ...
  const estimates = Object.entries(VEHICLE_CONFIGS).map(([key, config]) => {
    const price = calculatePrice(distanceKm, key as keyof typeof VEHICLE_CONFIGS)
    return {
      vehicleType: key,
      vehicleName: config.displayName,
      emoji: config.emoji,
