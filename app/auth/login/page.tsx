  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        // Redirect based on role - in production, read from profile
        if (role === 'driver') {
    ...
    <main className="min-h-screen bg-dark-900 bg-mesh flex flex-col">
      ...
      <div className="px-4 pt-12 pb-8 max-w-lg mx-auto w-full">
        ...
        <p className="text-white/40 text-sm">
          {mode === 'signin' ? 'Sign in to continue' : 'Join thousands of users in Assam'}
        </p>
      </div>
          ...
          <div className="space-y-2">
            {[
              ...
              <button
                key={demo.role}
                onClick={() => {
                  setForm({ ...form, email: demo.email, password: demo.pw })
                  setMode('signin')
                }}
                className="w-full text-left px-3 py-2 rounded-lg bg-dark-700 hover:bg-dark-600 transition-colors text-xs"
              >
